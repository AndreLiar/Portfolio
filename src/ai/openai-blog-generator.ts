...
// Configure Genkit instance for fallback
const ai = genkit({
  plugins: [
    googleAI({
...
// Technology detection and analysis functions (reused from enhanced generator)
...
function validateContentQuality(content: string): { isValid: boolean; issues: string[] } {
  ...
  // More specific template phrases that indicate truly generic content
  const templatePhrases = [
    'addresses modern development challenges',
    'leverages cutting-edge technology to create',
    'modern technology stack that addresses',
    'scalable architecture that leverages'
  ];
  ...
  // Only fail if content is very short AND has template language
  if (content.length < 800) {
    issues.push('Content too short (less than 800 characters)');
  }
  
  // Check for meaningful headings (more lenient)
  const headings = content.match(/#{2,6}\s+(.+)/g) || [];
  if (headings.length < 2) {
    issues.push('Insufficient section structure (need at least 2 headings)');
  }
  
  // Only fail if multiple issues or severe problems
  const criticalIssues = issues.filter(issue => 
    issue.includes('too short') || 
    issue.includes('Insufficient section structure') ||
    templateCount > 1
  );
...
// Main blog generation function with OpenAI primary
export async function generateBlogPost(input: BlogGenerationInputType): Promise<BlogPostOutputType> {
  try {
    ...
    console.log(`🚀 Starting OpenAI blog generation for ${githubUsername}/${repositoryName}`);
    
    // Step 1: Deep repository analysis
    const analysisData = await analyzeRepositoryDeep(githubUsername, repositoryName);
    
    // Step 2: Create enhanced prompt for OpenAI
...
`;
    ...
    // Step 3: Generate content with OpenAI (primary)
    console.log('🤖 Generating content with OpenAI GPT-3.5-turbo...');
    ...
    try {
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a senior technical writer who creates publication-ready, specific, and detailed blog posts about software projects."
          },
          {
            role: "user", 
      ...
      console.log(`📝 OpenAI generated ${responseText.length} characters`);
      ...
      if (jsonMatch) {
        ...
        if (!validation.isValid) {
          console.log('⚠️ OpenAI content quality issues:', validation.issues);
          throw new Error('OpenAI content quality validation failed');
        }
        
        console.log('✅ High-quality OpenAI content generated successfully');
      } else {
      ...
      console.log('⚠️ OpenAI API failed, falling back to Gemini:', openaiError.message);
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          const validation = validateContentQuality(blogData.content || '');
          if (!validation.isValid) {
            console.log('⚠️ Gemini content quality issues:', validation.issues);
            throw new Error('Gemini content quality validation failed');
          }
          
          console.log('✅ High-quality Gemini fallback content generated');
        ...
        console.log('⚠️ Both OpenAI and Gemini failed, using intelligent fallback');
        
        // Final fallback to intelligent content generator
...
// Intelligent fallback generator and helper functions (reused from enhanced generator)
function generateIntelligentFallback(analysisData: any, tone: string, focus: string, targetAudience: string) {
  ...
  return {
    ...
    estimatedReadTime: Math.ceil(content.length / 1000 * 4),
...
// Helper functions (reused from enhanced generator)
