...
// Define the Language type based on your data structure
interface Language {
  id: string;
  name: string;
  level: string;
...
interface AdminLanguagesClientProps {
  initialLanguages: Language[];
}

export function AdminLanguagesClient({ initialLanguages }: AdminLanguagesClientProps) {
  ...
  const confirmDeleteItem = async () => {
    ...
    try {
      await languageApi.deleteLanguage(itemToDelete);
      toast({
      ...
      toast({
        title: 'Error',
        description: 'Failed to delete the language. Please try again.',
        variant: 'destructive',
      });
    } finally {
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem asChild>
                        <Link href={`/admin/languages/${item.id}`}>
